...
export default class AnnouncementsPage extends React.Component {
    ...
    refresh() {
        ...
        getRequest.call(this, `article/get_articles?status=PUBLIC&type=ANNOUNCEMENTS&order=${this.state.filters}`, (data) => {
            this.setState({
                articles: data,
                loading: false,
            });
                    ...
                    {currentArticles ? (
                        currentArticles.length > 0 ? (
                            currentArticles.map((article) => {
                                ...
                                const imageClass = isDefaultImage ? "announcement-article-image-default" : "announcement-article-image-preview";

                                return (
                                    <div key={article.id} className="announcement-article-card fade-in">
                                        <Link to={`/article/${article.id}`}>
                                            <img
                                                ...
                                                alt={article.title}
                                            />
                                            <h2 className="announcement-article-title ml-n2">{article.title}</h2>
                                            <p className="announcement-publication-date"><i>{article.publication_date.split("T")[0]}</i></p>
                                            <div className="announcement-article-abstract">
