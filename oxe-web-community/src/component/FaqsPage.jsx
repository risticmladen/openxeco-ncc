...
export default class FaqsPage extends React.Component {
    ...
    refresh() {
        ...
        getRequest.call(this, `article/get_articles?status=PUBLIC&type=FAQ&order=${this.state.filters}`, (data) => {
            this.setState({
                articles: data,
                loading: false,
            });
            ...
            <div className={"PageHome page max-sized-page row-spaced"}>
                <div className={"row"}>
                    <div className="col-md-10 mb-5">
                        <h1 className="dashboard-header faq-header">Frequently Asked Questions</h1>
                    </div>
                </div>
                    ...
                    {currentArticles ? (
                        currentArticles.length > 0 ? (
                            currentArticles.map((article) => {
                                ...
                                const imageClass = isDefaultImage ? "faq-article-image-default" : "faq-article-image-preview";

                                return (
                                    <div key={article.id} className="faq-article-card fade-in">
                                        <Link to={`/article/${article.id}`}>
                                            <img
                                                ...
                                                alt={article.title}
                                            />
                                            <h2 className="faq-article-title ml-n2">{article.title}</h2>
                                            <p className="faq-publication-date"><i>{article.publication_date.split("T")[0]}</i></p>
                                            <div className="faq-article-abstract">
                            ...
                            <p>There are no FAQs at the moment.</p>
                        )
                    ) : (
                        <p>Loading...</p>
                    )}
